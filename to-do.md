...
- [ ] **Auth persistence 운영 정책 결정**: `VITE_AUTH_PERSISTENCE`를 `local` vs `session` 중 무엇으로 운영할지 보안/UX 기준으로 결정하고 문서화
  - **보안 체크**: 공용PC/태블릿(헬스장 키오스크) 사용 가능성, 로그아웃 유도 UX(명시적 로그아웃 버튼 가시성), 브라우저 데이터 삭제/시크릿 모드 영향
  - **UX 체크**: 개인 기기에서 “자동 로그인” 기대 수준, 재로그인 빈도 허용치, PWA 설치/재실행 동작
  - **권장안(기본)**: 개인용 서비스 중심이면 `local`, 공용기기 사용이 많으면 `session` 또는 공용기기 프로필에 한해 `session` 강제(운영 가이드로 구분)
